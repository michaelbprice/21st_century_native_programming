...
Board::Board ()
  : m_koState(make_pair(false, ref(m_points[0][0])))
{
    LOG_FUNCTION(wcout, L"Board::Board");
...
{
    LOG_FUNCTION(wcout, L"Board::doesKoRuleApply");
    ...
    if (potentiallyCapturedChain.size() == 1)
    {
        ...
        {
            gLogger.log(LogLevel::kMedium, wcout, L"The Ko rule was enforced"); // " for point ", m_koState.second);
            return true;
        }
    }
...
{
    LOG_FUNCTION(wcout, L"Board::getAllEmptyChains");
    ...
    for (size_t row = 0; row < m_points.size(); ++row)
    {
        for (size_t column = 0; column < m_points[row].size(); ++column)
        {
            ...
            try
            {
                ...
                gLogger.log(LogLevel::kMedium, wcout, L"Discovered empty chain"); // " : ", chain);
            }
            catch (const Chain::PointVisitedAlreadyException &)
            {
                gLogger.log(LogLevel::kHigh, wcout, L"Skipping point"); // " : ", point);
            }
        }
    }
...
{
    LOG_BUSY_FUNCTION(wcout, L"Board::getPointAbove");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Board::getPointBelow");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Board::getPointLeft");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Board::getPointRight");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Board::getStoneColorAt");
...
{
    LOG_FUNCTION(wcout, L"Board::isOccupiedPoint");
...
{
    LOG_FUNCTION(wcout, L"Board::isValidMove");
    ...
    {
        LOG_FUNCTION(wcout, L"Board::lambda_wouldCapture");
        ...
        try
        {
            ...
            gLogger.log(LogLevel::kMedium, wcout, L"Discovered chain"); // " : ", chain);
...
{
    LOG_FUNCTION(wcout, L"Board::placeStoneAt");
...
{
    LOG_FUNCTION(wcout, L"Board::removeCapturedStones");
        ...
        for (size_t column = 0; column < m_points[row].size(); ++column)
        {
            ...
            {                
                ...
                if (currentChain.color() == colorToCapture && currentChain.libertyCount() == 0)
                {
                    ...
                    {
                        LOG_BUSY_FUNCTION(wcout, L"Chain::removeCapturedStones::lambda_doStoneRemoval");
            ...
            {
                gLogger.log(LogLevel::kHigh, wcout, L"Skipping point", thePoint);
            }
        }
    }
