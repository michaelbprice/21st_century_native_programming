...
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::Chain(StoneColor,...");
 
    init(board, pointsToIgnore);
}

Chain::Chain (const Point & startPoint, const Board & board, ConstPointSet * pointsToIgnore)
  : m_color(startPoint.getStoneColor())
  , m_startPoint(startPoint)
  //, m_chainAndNeighbors({{StoneColor::NONE,  ConstPointSet()},
  //                       {StoneColor::BLACK, ConstPointSet()},
  //                       {StoneColor::WHITE, ConstPointSet()}})
{
    LOG_FUNCTION(wcout, L"Chain::Chain(...)");

    init(board, pointsToIgnore);
}

void Chain::init (const Board & board, ConstPointSet * pointsToIgnore)
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::init");
    ...
    {
        if (pointsToIgnore->find(&m_startPoint) != pointsToIgnore->end())
    ...
    doChainCalculation(m_startPoint, board, myPointsToIgnore); 
...
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::doChainCalculation");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::borderCountOf");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::color");
...
{
    LOG_FUNCTION(wcout, L"Chain::libertyCount");
...
{
    LOG_BUSY_FUNCTION(wcout, L"Chain::size");
...
{
    LOG_FUNCTION(wcout, L"Chain::containsPoint");
